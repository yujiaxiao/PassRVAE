...
class PassRVAE(nn.Module):
    ...
    def inference(self, n=10, z=None):
        ...
        while t < self.max_sequence_length and len(running_seqs) > 0:
            ...
            logits = self.outputs2vocab(output.squeeze(0))  # Remove the additional dimension

            input_sequence = self._sample(logits,k=30)
    ...
    def _sample(self, dist, k=30):
        values, indices = torch.topk(dist, k, dim=-1)
        probabilities = F.softmax(values, dim=-1)
        flattened_probs = probabilities.view(-1, k)
        sample_idx = torch.multinomial(flattened_probs, num_samples=1)
        sample = indices.view(-1, k).gather(dim=-1, index=sample_idx)
